...
CACHE_PATH_RESULTS = os.path.join(DATA_DIR, 'results.pkl')
CACHE_PATH_CALIB   = os.path.join(DATA_DIR, 'raw_data_calib.pkl')
...
if __name__ == '__main__':
    ...
    try:
        ...
        export_fill_stats_xlsx(
            ...
            path=os.path.join(SHEETS_DIR, 'fill_stats.xlsx')
        )
    except Exception as e:
    ...
    try:
        export_quote_position_xlsx(
            ...
            best_bid_dict,
            best_ask_dict,
            path=os.path.join(SHEETS_DIR, 'quote_position.xlsx')
        )
    except Exception as e:
        print(f"\nExport skipped: {e}")