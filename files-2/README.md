...
### 4. Fill Analysis (replaces Markov Chain from Section 4.2)
The paper used a Markov chain to derive spread-capture probabilities analytically
because its Poisson simulator made fills genuinely probabilistic — the geometric
waiting-time formula `p* = p(0,2) + Σ p(0,1)·p(1,1)ⁿ·p(1,2)` was a valid
analytic result under that model.

With TAQ replay, fills are deterministic given the historical data. Fitting a
Markov chain to the resulting frequencies and applying the same formula would
produce numbers that look like probabilities but aren't — the memoryless
transition assumption is violated because real market order flow has intraday
patterns and serial correlation.

We instead report direct empirical rates per quote cycle, averaged across days:
...
| `no_fill_rate` | Fraction of quote cycles with no fill |
| `fills_per_quote` | Average total fills per quote posted |
| `avg_spread_captured` | Average spread earned on two-sided fills ($) |
| `imbalance_ratio` | abs(buy_fills - sell_fills) / total_fills — lower is better for inventory control |

These are directly comparable to Tables 7 and 8 of the paper as performance
metrics, with a cleaner interpretation: observed rates over the specific week
simulated, not forward-looking probabilities.
