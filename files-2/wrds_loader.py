...
class WRDSLoader:
    ...
    def _process_day(self, quotes: pd.DataFrame,
        ...
        # Include bidsiz/asksiz for order book imbalance fill model
        q_resampled = quotes[['bid', 'ask', 'bidsiz', 'asksiz']].resample('1s').last().ffill()
        ...
        bid_sizes = q_resampled['bidsiz'].values.astype(float)  # shares at best bid
        ask_sizes = q_resampled['asksiz'].values.astype(float)  # shares at best ask
        ...
        return {
            ...
            'best_bid'      : best_bid,
            'best_ask'      : best_ask,
            'bid_sizes'     : bid_sizes,   # quoted size at best bid (for OBI)
            'ask_sizes'     : ask_sizes,   # quoted size at best ask (for OBI)
            'sigma'         : sigma,
